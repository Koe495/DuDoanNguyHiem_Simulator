...
class SimulationData:
    def __init__(self):
        ...
        self.auto_stand_timer = 0  # Bộ đếm tự đứng dậy nếu ngã nhẹ
    ...
    def update_body(self):
        # 1. LOGIC TÉ NGÃ VÀ TỰ ĐỨNG DẬY
        if self.is_fallen:
            if self.auto_stand_timer > 0:
                self.auto_stand_timer -= 1
                if self.auto_stand_timer == 0:
                    self.is_fallen = False  # Tự đứng dậy thành công
                    self.accel = 1.0
            else:
                self.fall_duration += 5  # Nằm bẹp (chấn thương nặng)
                self.accel = 1.0
        else:
            self.fall_duration = 0
        ...
        # 3. LOGIC THÂN NHIỆT (HIỆU ỨNG THỰC TẾ)
        temp_loss = (24.0 - self.env_temp) * 0.005

        if self.wearing_warm:
            # Mặc áo ấm: Chống mất nhiệt nếu trời lạnh, NHƯNG cộng thêm nhiệt nếu trời nóng
            temp_loss -= 0.08
...
class GameApp:
    def __init__(self):
        ...
        # Hệ thống nhập liệu Custom Environment
    ...
    def toggle_pause(self):
        ...
        self.active_input = None  # Hủy trạng thái nhập liệu khi unpause
    ...
    def handle_mouse_swipe(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.mouse_down_pos:
                ...
                if speed > 50:
                    # Nếu đang ngủ, tốc độ chuột bị chia đôi (Kháng ngã)
                    ...
                    if self.sim.accel < 3.0:
                        self.sim.auto_stand_timer = 2  # Ngã nhẹ, tự đứng dậy sau 2 tick
                        print(f"Ngã nhẹ ({self.sim.accel:.2f}G). Sẽ tự đứng dậy.")
                    else:
                        self.sim.auto_stand_timer = -1  # Chấn thương nặng, nằm im
                        print(f"CHẤN THƯƠNG ({self.sim.accel:.2f}G)! Cần cứu hộ.")

                self.mouse_down_pos = None

    def handle_custom_inputs(self, event):
        # Chọn ô nhập liệu khi Pause
        ...
        if event.type == pygame.KEYDOWN and self.active_input:
            if event.key == pygame.K_RETURN:
                ...
                # Chỉ cho phép nhập số, dấu phẩy/chấm và dấu trừ
    ...
    def draw_player(self):
        ...
        if self.sim.is_fallen:
            ...
            # Fix Lỗi Crash: Vẽ trang bị nằm ngang khi ngã
    ...
    def draw_environment_ui(self):
        # Đổi màu nền theo Target Môi trường
        bg_color = BG_NORMAL
        ...
        for key, text in params:
            ...
            if self.is_paused and self.active_input == key:
                pygame.draw.rect(self.screen, YELLOW, rect)  # Highlight ô đang nhập
                self.screen.blit(font.render(self.input_text + "_", True, BLACK), (rect.x + 5, rect.y))
            else:
                # Nếu Pause thì kẻ khung nhẹ để biết là click được
    ...
    def run(self):
        while True:
            ...
            if self.state == "MENU":
                ...
                fall_status = "Đang đứng" if self.sim.fall_duration == 0 else f"{self.sim.fall_duration} s"
                self.screen.blit(font.render(f"Fall duration: {fall_status}", True, BLACK), (30, 150))
                ...
                # Nút Pause chớp nháy nếu đang Pause
                ...
                if self.sim.ai_status == 1:
                    ...
                    self.screen.blit(font.render("Caution: Hidden physiological risks.", True, WHITE),

...
# Load AI Model
try:
    rf_model = joblib.load('rf_model_v5.pkl')
    print("✅ Đã load mô hình AI thuần túy thành công.")
except:
    rf_model = None
    print("⚠️ Không tìm thấy rf_model.pkl hoặc lỗi định dạng. Chạy chế độ Toán học Dự phòng (Fallback Model).")
...
class SimulationData:
    ...
    def update_body(self):
        ...
        # 3. LOGIC HRV (BIẾN THIÊN NHỊP TIM - ĐO LƯỜNG STRESS)
        target_hrv = self.base_hrv
        if self.is_heavy_exercise: target_hrv -= 25
        if self.is_fallen: target_hrv -= 35
        ...
        if self.body_temp > 39.0 or self.body_temp < 35.0: target_hrv -= 25
        ...
        if self.wearing_warm: temp_loss -= 0.08
        if self.is_heavy_exercise: temp_loss -= 0.05
    ...
    def predict_danger(self):
        ...
        if rf_model is not None:
            try:
                # Không có bất kỳ câu lệnh if-else vượt ngưỡng cố định nào ở đây!
                prediction = rf_model.predict(df)[0]
                return int(prediction)
            except Exception as e:
                print(f"⚠️ Lỗi dự đoán AI: {e}")
                return 0
        else:
            # Nếu chưa có file pkl, sử dụng phương pháp toán học khoảng cách (Heuristic Distance)
            # để tính toán độ lệch sinh lý liên tục, mô phỏng cách hoạt động của mạng neural/AI.
            return self._fallback_mathematical_model()

    def _fallback_mathematical_model(self):
        """Mô hình toán học liên tục tính toán vector lệch sinh lý thay thế cho if-else cứng."""
        deviation_score = 0.0

        # Lệch thân nhiệt (Bình thường là 36.8)
        deviation_score += (abs(self.body_temp - 36.8) / 1.5) ** 2
        # Thiếu hụt Oxy
        if self.oxygen < 20.9:
            deviation_score += ((20.9 - self.oxygen) / 2.5) ** 2
        # Ô nhiễm không khí AQI
        if self.aqi > 100:
            deviation_score += ((self.aqi - 100) / 150) ** 2
        # Suy kiệt hệ thần kinh (HRV sụt giảm dữ dội)
        if self.hrv < self.base_hrv:
            deviation_score += ((self.base_hrv - self.hrv) / 20.0) ** 2
        # Thời gian bất tỉnh do chấn thương nặng
        if self.fall_duration > 0:
            deviation_score += (self.fall_duration / 15.0) ** 2

        # Phân loại trạng thái động dựa trên tổng điểm nguy cơ sinh học tích lũy
        if deviation_score >= 4.0: return 2  # Nguy hiểm cấp tính (Kích hoạt đếm ngược chết)
        if deviation_score >= 1.2: return 1  # Bất ổn (Warning)
        return 0
...
class GameApp:
    def __init__(self):
        ...
        pygame.display.set_caption("Explorer Simulator - Pure AI Survival")
    ...
    def handle_mouse_swipe(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if player_rect.collidepoint(event.pos): self.mouse_down_pos = event.pos
            ...
            if self.mouse_down_pos:
                ...
                if speed > 50:
                    ...
                    self.sim.auto_stand_timer = 2 if self.sim.accel < 3.0 else -1
                self.mouse_down_pos = None
    ...
    def handle_custom_inputs(self, event):
        ...
        if event.type == pygame.KEYDOWN and self.active_input:
            if event.key == pygame.K_RETURN:
                ...
                if event.unicode in '0123456789.-': self.input_text += event.unicode
    ...
    def draw_player(self):
        ...
        if self.sim.is_fallen:
            ...
            if self.sim.wearing_warm: pygame.draw.rect(self.screen, ORANGE, (x - 30, y + 70, 60, 25))
            if self.sim.wearing_mask: pygame.draw.circle(self.screen, BLUE, (x + 60, y + 80), 8)
        else:
            ...
            pygame.draw.line(self.screen, color, (x, y - 40 + offset), (x + 30, y - 10 + offset), 5)
            if self.sim.wearing_warm: pygame.draw.rect(self.screen, ORANGE, (x - 15, y - 60 + offset, 30, 60))
            if self.sim.wearing_mask: pygame.draw.circle(self.screen, BLUE, (x + 12, y - 80 + offset), 8)

    def draw_environment_ui(self):
        bg_color = BG_NORMAL
        env_name = "IDEAL ENVIRONMENT"
        if self.sim.target_env_temp <= 5.0:
            bg_color = BG_SNOW; env_name = "SNOW MOUNTAIN"
        elif self.sim.target_aqi >= 150.0:
            bg_color = BG_CAVE; env_name = "HAZARDOUS CAVE"
    ...
    def run(self):
        while True:
            ...
            if self.state == "MENU":
                ...
                title = large_font.render("SIMULATION: PURE AI SURVIVAL PREDICTOR", True, BLACK)
                self.screen.blit(title, (WIDTH // 2 - title.get_width() // 2, 100))

                self.draw_button("-", 300, 280, 50, 40, GRAY,
                                 lambda: setattr(self.sim, 'base_hr', max(50, self.sim.base_hr - 5)))
                self.screen.blit(font.render(f"Base heart rate: {self.sim.base_hr} bpm", True, BLACK), (380, 290))
                ...
                self.screen.blit(font.render(f"Base HRV (Stress): {self.sim.base_hrv} ms", True, BLACK), (380, 350))
                ...
                if current_time - self.last_tick > TICK_RATE and not self.is_paused:
                    ...
                    # AI THUẦN TÚY QUYẾT ĐỊNH TRẠNG THÁI SINH TỒN
                    self.sim.ai_status = self.sim.predict_danger()

                    if self.sim.ai_status == 2:
                        self.sim.danger_timer += 1
                        if self.sim.danger_timer >= 10: self.state = "GAMEOVER"
                    else:
                ...
                # HIỂN THỊ CẢNH BÁO DO AI TRẢ VỀ
                if self.sim.ai_status == 1:
                    ...
                    self.screen.blit(font.render("AI detected physiological / ambient stress.", True, WHITE),
                    ...
                    self.screen.blit(large_font.render("WARNING! CRITICAL STATE!", True, WHITE),
                                     (alert_box.x + 10, alert_box.y + 10))
                    self.screen.blit(
                        font.render(f"Critical AI Risk! Evacuate / Heal. Death in: {10 - self.sim.danger_timer}s", True,
                                    WHITE), (alert_box.x + 10, alert_box.y + 50))

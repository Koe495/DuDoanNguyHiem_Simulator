...
class SimulationData:
    def __init__(self):
        self.age = 25
        self.base_hr = 75.0
        self.base_hrv = 60.0  # Thêm Baseline HRV
    ...
    def update_body(self):
        ...
        if self.is_fallen: target_hrv -= 30  # Chấn thương gây stress tột độ

        # Môi trường khắc nghiệt làm giảm HRV
        if self.aqi > 100: target_hrv -= 15  # Khí độc ức chế thần kinh
        if self.oxygen < 19 and not self.wearing_mask: target_hrv -= 20
        if self.env_temp < 10 and not self.wearing_warm: target_hrv -= 15
        if self.env_temp > 35: target_hrv -= 15

        # Ngủ/Nghỉ ngơi làm tăng HRV (Phục hồi)
        if self.is_sleeping: target_hrv += 20

        # Biến thiên mượt mà
        self.hrv += (target_hrv - self.hrv) * 0.1 + random.uniform(-1, 1)
        self.hrv = np.clip(self.hrv, 10.0, 100.0)  # Ngưỡng sinh lý hợp lý
    ...
    def run(self):
        while True:
            ...
            if self.state == "MENU":
                ...
                # 2. Khung Sinh lý (Tăng chiều cao hộp để hiển thị HRV)

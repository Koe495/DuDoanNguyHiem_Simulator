# genData8.py
"""
Dataset Generator v8 – Survival Risk Simulation
================================================
Thay đổi so với v7:
  - XOÁ dehydration hoàn toàn: không còn là biến trạng thái, không ghi vào CSV,
    không tác động lên HR/HRV/body_temp (dehydration không có trong dataset thực)
  - UV tác động trực tiếp vào _physio_step: body_temp và HRV
  - fall_duration trong CRIT_SEVERE_FALL giới hạn tối đa 30s (khớp gameplay)
    và trauma_rate tăng 10× để đạt ngưỡng nguy kịch trong thời gian ngắn
  - Xuất: simulation_survival_dataset8.csv + scenarios.json
"""
...
# ── File đầu ra ───────────────────────────────────────────────────────────────
OUTPUT_CSV  = "simulation_survival_dataset8.csv"
...
def _init_env(name: str) -> dict:
    ...
    if name == "NORMAL_WALKING":
        ...
        p["trauma_rate"] = 0.04 * SCALE     # 0.0016 /step → trauma ≈ 0.8 sau 500 bước (đủ nhãn 1)
...
def _derive_label(body_temp: float, oxygen_env: float, aqi_env: float,
                  ...
                  uv_index: float, env_temp: float,
                  base_hr: float = 70.0, base_hrv: float = 62.0,
                  hr: float = 70.0) -> int:
    """
    Tính biological_deviance cá nhân hoá rồi gán nhãn có vùng mờ.
    Đã loại bỏ dehydration (không có trong dataset thực).
    UV tác động trực tiếp vào deviance qua uv_heat.
    """
    # -------------------------------------------------------------------------
    # THÊM LUẬT CỨNG
    # Bất chấp các chỉ số khác ra sao, nếu thân nhiệt quá thấp (<= 33)
    # hoặc quá cao (>= 40), hệ thống sẽ lập tức trả về nhãn 2 (Nguy kịch).
    if body_temp <= 33.0 or body_temp >= 40.0:
        return 2
    # -------------------------------------------------------------------------
...
def _physio_step(hr: float, hrv: float, body_temp: float,
                 ...
                 oxygen_env: float, aqi_env: float,
                 uv_index: float = 0.0) -> tuple:
    """
    Tiến một bước TIME_STEP_S. Trả về (hr, hrv, body_temp).
    Đã loại bỏ dehydration. UV tác động vào body_temp và HRV.
    """
    # UV thực tế hấp thụ (clothing chặn 60%)
    uv_exposed = uv_index * (1.0 - clothing * 0.6)
    uv_excess  = max(0.0, uv_exposed - 3.0)

    # A. Thân nhiệt
    hp      = 0.012 * (1.0 + exertion) * SCALE
    hl      = 0.004 * (env_temp - body_temp) * (1.0 - clothing) * SCALE
    uv_heat = uv_excess * 0.00018 * SCALE
    body_temp = float(np.clip(
        body_temp + hp + hl + uv_heat + random.gauss(0, 0.004),
        30.0, 43.0
    ))
    ...
    target_hr = base_hr + exertion * 65.0 + hypoxia_d + toxic_d + shock_d
    hr = float(np.clip(
    ...
    target_hrv = (base_hrv - exertion * 20.0
                  - thermal_s - hypoxia_s - toxic_s - injury_s - uv_s)
    hrv = float(np.clip(
...
def simulate_user_timeline(scenario_name: str, num_events: int,
    ...
    for ev in range(num_events):
        ...
        base_hr  = random.uniform(50.0, 90.0)
        base_hrv = random.uniform(25.0, 90.0)
        ...
        FRAME_SCALE = 25   # 25 bước × 0.2s = 5s (khớp frame gốc)
        FALL_MAX_S  = 30   # fall_duration tối đa 30s (khớp gameplay)
        ...
        for i in range(rows_per_event):
            ...
            if scenario_name == "NORMAL_JUMPING" and (i % FRAME_SCALE == 0) and old_frame in [20, 50, 80]:
                ...
                # Không còn dehydration — exertion + nhiệt đã đẩy HR/HRV
                pass

            elif scenario_name == "CRIT_SEVERE_FALL":
                if i == 0:
                    accel = random.uniform(6.5, 9.0)
                # Giới hạn fall_duration tối đa FALL_MAX_S để khớp gameplay
                fall_duration = min(int(i * TIME_STEP_S), FALL_MAX_S)
            ...
            data.append([
                ...
                round(base_hr, 1), round(base_hrv, 1),
                derived_label,
            ])
...
def simulate_composite_scenario(scenario_a: str, scenario_b: str,
    ...
    Trạng thái sinh lý (hr, hrv, body_temp, trauma_shock)
    ...
    for ev in range(num_events):
        ...
        for i in range(rows_per_event):
            ...
            if i >= transition_at and "FALL" in scenario_b:
                fall_duration = min(int((i - transition_at) * TIME_STEP_S), FALL_MAX_S)
                trauma_shock  = min(1.0, trauma_shock + 0.008)  # tăng 10× khớp gameplay
...
if __name__ == "__main__":
    print("=== BẮT ĐẦU KHỞI TẠO DATASET PHẢN HỒI SINH LÝ V8 ===")
